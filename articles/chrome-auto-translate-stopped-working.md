...
While in the language settings, review the list of languages you have added. Remove any languages you do not actually read, since Chrome will not offer translation for content in those languages. If you see an unexpected language entry, deleting it restores normal translation behavior for that language.

Click the "Manage languages" link and review the exception lists. Remove any entries for languages or sites where you want auto-translate re-enabled.
...
Click Clear Data, then close Chrome entirely using Ctrl+Shift+Q on Windows or Cmd+Q on Mac. Reopen Chrome and visit a foreign-language page to test. A full restart ensures Chrome reinitializes its translation service connections with fresh cache files.
...
If you see blocked requests, the fix requires adjusting your firewall or VPN settings, or testing on a different network. If translation works on a personal mobile hotspot but not on your corporate network, the network configuration is the cause.
...
> "Chrome translation failures caused by extension conflicts are most reliably diagnosed by testing in incognito mode, where extensions are disabled by default, then re-enabling them one by one."
>
...
> "The tested 6 best Chrome translation extensions showed that third-party extensions with their own API infrastructure outperform Chrome's built-in translation in enterprise and restricted network environments."
>
...
Chrome's built-in translation respects site-level blocking. Third-party extensions like BeLikeNative can translate content on blocked sites by using separate translation APIs that operate independently of Chrome's restriction system.
