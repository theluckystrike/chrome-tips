...
description: "Learn how to delete individual autofill suggestions in Chrome. Simple steps to remove unwanted entries without clearing everything. Read more to optimize your e"
date: 2026-03-09
...
The problem is that Chrome does not always know when information becomes outdated or unwanted. An old email address you used once for a newsletter might still appear in your autofill suggestions months later. A shipping address from a previous home might keep showing up when you are trying to complete a new order. Sometimes you might accidentally save incorrect information, or you might share your computer with someone else and do not want their information mixing with yours.

Autofill suggestions come from two main sources. The first is your browsing history and the forms you have filled out over time. Every time you enter information into a website, Chrome learns from this and adds it to your autofill database. The second source is when you explicitly save information through Chrome is settings. Both sources can create suggestions that you might want to delete later.
...
For passwords that Chrome has saved, start by clicking on your profile picture in the upper right corner of Chrome and selecting the key icon or going to Settings and then clicking on Passwords. You will see a list of all saved passwords. Browse through the list or use the search bar to find the specific entry you want to delete. When you find it, click on the three dots next to that entry and select Delete. Chrome will ask you to confirm, and once you do, that password will be gone. This method works for removing individual passwords without affecting any of your other saved credentials.

For addresses and other form data, the process is slightly different. Go to Chrome Settings and look for the section called "Addresses and more" or "Payment methods" depending on what you want to edit. Click on it to see all your saved addresses. You will see each address listed, and next to each one, you should see a three-dot menu or an option to edit or delete. Click on Delete for the specific address you want to remove. If you only want to remove one address but keep others, this lets you pick exactly which ones to delete.
...
To access this feature, click on the three dots in the upper right corner of Chrome, then select Settings. On the left sidebar, click on Privacy and security, and then click on Clear browsing data. You can also press Ctrl+Shift+Delete on Windows or Cmd+Shift+Delete on Mac to open this directly.

In the window that appears, look at the checkboxes. You will see options for browsing history, cookies and other site data, cached images and files, and more. For autofill data, make sure the checkbox for "Browsing history" is selected. You might also want to look for "Autofill form data" if it appears as a separate option, though this varies by Chrome version.

Use the dropdown menu at the top to select the time range. If you want to remove all autofill suggestions, select "All time." This will clear everything, so keep in mind this will also remove any autofill data you actually want to keep. If you only want to remove recent suggestions, choose a shorter time period.
...
You can choose to turn off autofill entirely if you prefer to type everything manually. However, this defeats the convenience that makes autofill useful. A better approach is to periodically review your saved addresses and payment methods and delete the ones you no longer need. This keeps your autofill suggestions relevant and reduces the chance of accidentally using outdated information.
...
For example, Tab Suspender Pro is an extension that helps Chrome users manage their tabs more effectively. While it does not directly manage autofill, it addresses a related problem many Chrome users face: having too many tabs open. When you have dozens of tabs, Chrome can become slow and unresponsive, which might make managing autofill settings feel more frustrating. Tab Suspender Pro automatically suspends tabs you are not using, freeing up memory and making Chrome feel faster. It also provides a convenient sidebar to see all your open and suspended tabs, making it easier to find what you need without cluttering your browser.
