...
Before you can use the chrome.alarms API, you need to declare it in your extension's manifest file. If you are using Manifest V3, which is the current standard, you will need to add the "alarms" permission to your manifest. a name for the alarm and an object that specifies when it should fire. For a one-time alarm, you can set the delayInMinutes property to specify how many minutes from now the alarm should fire.
