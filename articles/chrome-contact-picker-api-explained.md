...
description: "Learn what Chrome Contact Picker API is, how websites use it, and how Check out our complete 2026 guide for more details and professional advice on managing ..."
date: 2026-03-09
...
Instead of typing in an email address or phone number manually, you can select from your existing contacts with just a few taps or clicks. This saves time and reduces errors when sharing links, files, or invitations through web applications. The API was introduced to make web apps feel more like native apps while still giving users control over their data.
...
The API makes web apps more convenient by giving them a safe way to access the contact information you already have on your device. When developers build features like sharing a document, sending a invitation, or recommending a product to a friend, they can use the Contact Picker API to let you select recipients from your actual contacts rather than asking you to type everything manually.
...
Common uses for this API include email marketing tools that let you add recipients from your contacts, social media apps that help you invite friends to events or share content, file sharing services that let you send documents to specific people, and online calendars that make it easy to invite contacts to events. Each time you use one of these features, Chrome will ask for your permission before showing your contacts, and you always have the choice to deny the request.
...
To review or change contact permissions, open Chrome and type chrome://settings in the address bar. Scroll down to the Privacy and Security section and click on Site Settings. Look for the Contacts option in the list. Here you can see which websites have requested contact access and change your preferences.

You have three main options for contact permissions. You can allow all websites to ask for contacts, which means every time a website wants to use the Contact Picker API, Chrome will ask for your permission. You can block all websites from accessing contacts, which will prevent any website from using the Contact Picker API. Or you can let Chrome ask each time, which is the default and generally recommended setting.
...
However, it is still smart to be thoughtful about which websites you grant contact access to. Only allow contact access to websites you trust and that have a legitimate reason for needing your contacts. If a website asks for contacts but the reason is not clear, it is perfectly reasonable to deny the request and look for an alternative way to accomplish what you need.

You should also be aware that some websites might try to use contact access for marketing purposes, such as inviting all your contacts to join a service or sending invitations on your behalf. Only share contacts when you are comfortable with how the website might use that information.
...
One common issue is that the Contact Picker does not appear when you try to use a feature that requires it. This usually means contact permissions are blocked. Check your Chrome settings as described above and make sure contact access is allowed or set to ask each time.

Another problem is that your contacts do not appear in the picker. This could mean your contacts are not synced to your Google account or device. The Contact Picker typically accesses contacts from your default Google account, so make sure your contacts are properly saved and synced.

If the website cannot read the contact information you selected, there might be a compatibility issue between the website and Chrome. In this case, you might need to manually enter the contact information or use an alternative method the website provides.
