...
The anchor positioning API in Chrome has been in development for several years and is now available in modern versions of Chrome (version 125 and later). This feature is a significant improvement for developers who want to create responsive, accessible UI components without relying on heavy JavaScript libraries.
...
Popovers and modals can also use anchor positioning. You can create a popover that appears next to a button and automatically repositions itself if it would go off-screen. This is especially useful for floating action buttons, notification panels, and context menus.
