...
description: "Learn how to allow popups for one site in Chrome without disabling your popup blocker completely. Simple steps inside. Check out our expert recommendations and"
date: "2026-01-15"
last_modified_at: "2026-03-09"
permalink: "chrome-how-to-allow-popups-for-one-site"
categories: [browser, settings]
tags: [chrome, popups, browser-settings, permissions]
...
First, open Chrome and navigate to the website where you want to allow popups. Once you are on the site, look at the address bar on the left side of the browser window. You should see an icon that looks like a window with an X through it if any popups have been blocked. This icon might be hard to see, so look carefully near the website address.

Click on that icon. A small menu will appear showing you how many popups have been blocked from this site. The menu will give you the option to always allow popups from this website. Click on the option that says something like "Always allow popups from [website name]" or "Always allow on this site."
...
Open Chrome and click the three dots in the upper right corner of the window. This opens the Chrome menu. From the menu, select Settings. On the Settings page, look for Privacy and security in the left sidebar and click on it. In the main area of the page, find Site Settings and click on it.

On the Site Settings page, scroll down until you see the Permissions section. Look for Popups and redirects in this list and click on it. You will see two sections: one showing websites that are allowed to open popups and another showing websites that are blocked.

To add a new website to the allowed list, click the Add button next to the Allow section. A small window will appear where you can type the web address of the site you want to allow. Make sure you enter the exact address, including whether the site uses http or https. For example, if you want to allow popups from example.com, you would type "https://example.com" or just "example.com" depending on how the site works. After entering the address, click Add to save your changes.
...
First, check if you have any extensions installed that block popups. Some ad blockers and privacy extensions have their own popup blocking features that work independently of Chrome settings. If you have such an extension, you might need to allow popups for that specific site within the extension settings as well.

Second, make sure you entered the website address correctly when adding it to the allowed list. If the site uses subdomains, you might need to add each subdomain separately. For example, if you allowed popups for example.com but the popup comes from app.example.com, you would need to add that subdomain as well.
...
If you are using Chrome at work or school, some organizations configure browser policies that limit what you can change in settings. In such cases, you might not be able to modify popup settings yourself, and you would need to contact your IT administrator for help.
...
It is also a good idea to periodically review which sites you have allowed to open popups. If you notice a site in your allowed list that you do not remember adding, or that you no longer use, consider removing it. This keeps your browser configuration clean and reduces any potential security exposure.

Chrome popup settings are designed to give you control while still protecting you by default. By allowing popups only for specific sites when needed, you get the best of both worlds: functionality from the sites you trust and protection from unwanted windows everywhere else.
