...
Render-blocking resources also slow things down. These include JavaScript files and CSS stylesheets that must load before the page can display content. When these resources are large or numerous, they create a bottleneck thatthe largest contentful paint.
...
The Performance panel in DevTools offers deeper insights. Record a page load to see exactly when each element renders and identify bottlenecks. Look for the LCP marker in the timing track to understand what isyour largest contentful paint.
