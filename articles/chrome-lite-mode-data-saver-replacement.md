...
description: "Chrome Lite Mode is gone from desktop Chrome. Learn what happened and find simple solutions to save data while browsing. Read more to optimize your experience."
date: 2026-01-15
...
The feature was especially popular in regions where internet speeds were slower or data was expensive. It worked similarly to Opera Turbo mode and could significantly reduce the amount of data you used while browsing. Many users found it invaluable for everyday web browsing, particularly when using public WiFi networks or when traveling.
...
The first option is Chrome's Data Saver setting, which is still available on Chrome for Android. On desktop Chrome, you can achieve similar results by adjusting your settings. Go to Chrome Settings, then Privacy and Security, and look for options to limit background data usage and disable JavaScript on certain sites.

Another built-in feature is the ability to block images selectively. By going to Site Settings, you can configure Chrome to not load images on specific websites, which significantly reduces data usage. You can also disable autoplay videos to prevent unwanted data consumption from video content.
...
Tab Suspender Pro is one solution that many users have found helpful. While its primary purpose is to suspend inactive tabs (similar to what the Great Suspender did before it was removed), this approach also saves memory and can reduce data usage. When tabs are suspended, they are essentially paused and not actively downloading content or using bandwidth. This means if you tend to keep many tabs open, suspending them can significantly reduce your overall data consumption.

The way Tab Suspender Pro works is simple. After you install the extension, you can configure it to automatically suspend tabs that have been inactive for a certain period. The suspended tabs remain visible in your tab bar but appear grayed out. When you click on a suspended tab, it quickly reloads so you can continue browsing. This is useful because you do not have to manually close tabs to save resources.
...
First, be mindful of video streaming. Services like YouTube, Netflix, and other video platforms use significant amounts of data. When possible, lower the video quality or download content for offline viewing during times when you have unlimited access to WiFi.

Second, pay attention to website notifications. Many websites ask for permission to send you notifications, which can use data in the background. Go to Chrome Settings, then Notifications, and revoke permission for sites you do not actively need notifications from.

Third, use the Chrome Task Manager to identify tabs using excessive resources. Right-click on the tab bar and select Task Manager to see which tabs are using the most memory and network data. You can then close or suspend tabs that are consuming too much.
