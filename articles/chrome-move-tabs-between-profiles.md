...
description: "Learn how to move tabs between Chrome profiles with simple step-by-step methods. Transfer tabs quickly between work and personal profiles. Read more to optimize"
date: 2026-01-15
...
First, organize the tabs you want to move into a new bookmark folder. Right-click on any tab in the group and select "Add all to bookmarks." Create a new folder with a clear name like "Tabs to Move" and save it. Make sure to check the box that says "Also save your open tabs from other devices" if you want to transfer all instances.

Now switch to your target profile. Open the bookmarks manager by pressing Ctrl+Shift+O (Windows) or Cmd+Shift+O (Mac). Navigate to the folder you just created. You will see all your tabs listed there, each as a bookmark that will open the exact same page when clicked.
...
Open Chrome using your first profile, then open a second Chrome window and switch to your second profile. You can do this by clicking your profile icon and selecting "Add" to create a new window, then choosing the other profile. Arrange the windows side by side on your screen.

Now simply drag tabs from one window to the other. Click and hold on a tab, then drag it into the other window. The tab will transfer to that profile. You can drag multiple tabs by holding Ctrl or Cmd while clicking to select several tabs first, then dragging them together as a group.
...
Tab Suspender Pro is one option that includes profile-aware tab management features. While its main purpose is automatically suspending inactive tabs to save memory, it also provides convenient options for organizing and transferring tabs across profiles. The extension adds a simple right-click menu option to move tabs to specific profiles, making the process much faster than manual methods.

Another popular choice is Session Buddy, which saves your tab sessions and lets you restore them into any profile. You can save your current tabs as a session, switch profiles, and then load that session into the new profile. This is particularly useful if you have many tabs to transfer and want to do it in one go rather than individually.
...
On your source profile, add all the tabs you want to transfer to bookmarks, as described in Method 1. Make sure sync is turned on for bookmarks in that profile's settings. Now sign into Chrome with your target profile on the same or a different computer. The bookmark folder will appear automatically through sync.
...
To export, install a simple extension like "Tab Export" or use Chrome's built-in bookmark export feature. Go to Bookmarks Manager, click the three-dot menu, and select "Export bookmarks." Save the HTML file.

To import into your target profile, go to the same menu and choose "Import bookmarks." Select the HTML file you saved. Your tabs will appear as a new folder in bookmarks that you can then open in the new profile.
...
The bookmark method is the most reliable fallback if you ever encounter issues with extensions or need to transfer tabs on a computer where you cannot install new software.
