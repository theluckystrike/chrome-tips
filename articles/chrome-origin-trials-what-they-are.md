...
description: "Learn what Chrome origin trials are, why they matter for web developers, and how to enable them to test new browser features before everyone else. Read more to "
date: 2026-01-15
...
Origin trials allow website owners and sometimes regular users to enable experimental features in Chrome. These features are not yet part of the standard browser. They are being tested in the real world to see how well they work. The feedback from origin trials is incredibly valuable. It helps Google identify bugs, understand how features affect website performance, and decide whether a feature is ready for everyone.
...
There are a few different types of origin trials. Some are for website developers only, where the feature only works on specific websites that have been set up for testing. Other origin trials can be enabled by any Chrome user through a simple opt-in process. The type of trial determines who can participate and how.
...
The dashboard shows whether a trial is open for new participants, whether it requires a token to be added to a website, and when the trial is scheduled to end. You can browse through the list to see if there are any features that interest you or that you might want to test.
...
The first method involves visiting the Chrome Origin Trials dashboard and looking for trials that are open to all users. Some trials allow any Chrome user to enable them directly. In these cases, you will see an option to register on the dashboard. Once you register, you receive a token that Chrome automatically recognizes. When you visit websites that support the feature, it will be enabled for you.

The second method is slightly more involved but gives you access to even more experimental features. You can enable Chrome flags, which are experimental settings that are not yet ready for public testing but are available for users who want to explore. To access Chrome flags, type chrome://flags in your address bar and press Enter. This opens a page with many experimental settings you can turn on or off.
...
Some websites might not work correctly with experimental features enabled. If you encounter problems on a particular website, try disabling the origin trial feature to see if that fixes the issue. Website developers are still learning how to use these new features correctly, so occasional incompatibilities are expected.
...
When a trial ends, the experimental feature will stop working until you remove the token or until Google releases it as a permanent feature. You do not need to do anything special when a trial ends. Chrome will simply stop supporting the feature until it either becomes official or is abandoned.
