...
faq:
  ...
  - q: "Why does Chrome use so much memory with multiple tabs?"
    a: "Chrome uses a multi-process architecture where each tab runs in its own process with its own browser engine, JavaScript interpreter, and security sandbox. A typical session with 20 tabs consumes 4-8GB of RAM, and background processes for extensions add another 500MB-2GB. While this isolation prevents one crashed tab from killing your browser, it creates significant memory overhead that compounds with every open tab."
  - q: "What causes memory leaks in Chrome tabs?"
    a: "Modern single-page applications built with React, Angular, or Vue often contain memory leaks that accumulate during extended browsing sessions. These leaks compound when you have multiple tabs open to memory-intensive sites like social media platforms, online IDEs, or data visualization tools. The Page Lifecycle API allows browsers to freeze and discard background tabs, but leaks continue to grow in active tabs. Zovo suggests refreshing memory-heavy sites periodically."
  - q: "How much RAM does each Chrome tab use?"
    a: "Chrome uses approximately 150-300MB per tab for typical websites, but JavaScript-heavy frameworks can push a single tab over 1GB of RAM. With 20 tabs open, Chrome can consume 4-8GB depending on site complexity. This exponential memory multiplication comes from Chrome's multi-process architecture, where each tab runs its own isolated process with separate browser engine and security sandbox."
  - q: "Does closing tabs fix chrome out of memory error?"
    a: "Yes, closing unused tabs is the fastest fix for chrome out of memory error. This resolves approximately 90% of memory-related crashes within 30 seconds. Since Chrome uses 150-300MB per tab minimum, closing even a few tabs can free substantial memory. Enable automatic tab discarding in chrome://settings/performance to let Chrome automatically manage background tab memory usage and prevent future errors."
---
