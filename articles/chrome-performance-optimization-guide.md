...
faq:
  ...
  - q: "How many processes does Chrome use per tab and extension?"
    a: "Chrome uses a separate process for every tab, extension, and utility running in the browser. This multi-process architecture enhances security and stability—if one tab crashes, others remain unaffected. However, this design requires significant memory, with each process consuming 50-300MB depending on page complexity. Tools like Zovo can help monitor process usage, and power users following the chrome performance optimization guide can use advanced flags to gain more granular control over process allocation."
  - q: "What is the best chrome performance optimization guide for reducing memory?"
    a: "The most effective chrome performance optimization guide organizes techniques by impact and difficulty, starting with simple built-in settings like enabling tab discarding and reviewing extension usage. The author, who maintains 16 extensions, recommends beginning with easy wins before moving to advanced options like command-line switches and experimental flags. Zovo emphasizes that measurable results come from understanding Chrome's resource allocation patterns and applying targeted optimizations."
  - q: "How much RAM does Chrome use with 30 open tabs?"
