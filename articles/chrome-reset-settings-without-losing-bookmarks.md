...
description: Need to reset Chrome to default settings? Learn how to restore Chrome
  without losing your bookmarks, passwords, and extensions.
date: 2026-01-15
...
- Homepage, startup page, and new tab page are reset
- Search engine settings return to Google
- Pinned tabs are unpinned
- Content settings like JavaScript and cookies permissions are reset
- Extensions are disabled (but not removed)
- Chrome flags are reset to default values
...
First, re-enable your extensions one by one. Go to the three-dot menu, select Extensions, then Manage Extensions. Turn on your extensions starting with the most essential ones. If you notice performance issues returning after enabling a particular extension, consider removing it or looking for a lighter alternative.

Second, check your sync settings. If you use Chrome sync, make sure you are signed in and that your bookmarks and other data have synced properly. Click the three dots, go to Settings, and ensure your Google account is connected under You and Google Sync.

Third, consider installing Tab Suspender Pro if memory usage was a concern before the reset. This extension automatically suspends inactive tabs to free up RAM, keeping Chrome running smoothly even with many open tabs. It is a great way to maintain the performance gains from your reset.
...
Before going this route, back up your bookmarks by going to the three-dot menu, selecting Bookmarks, then Bookmark Manager. From the three-dot menu in the manager, choose Export bookmarks to save an HTML file. After reinstalling Chrome, you can import these bookmarks using the Import bookmarks and settings option in the same menu.
...
For users who frequently keep many tabs open, consider using extensions that manage tab memory. This prevents Chrome from consuming excessive resources and keeps your browsing experience fast and responsive.
