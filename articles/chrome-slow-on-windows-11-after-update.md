...
description: Is Chrome running slow on Windows 11 after an update? Here are practical,
  step-by-step solutions designed for computers with limited RAM and older hardware.
date: 2025-02-16
categories:
- performance
- windows
...
First, Windows updates often reset your power settings. Your computer might switch from high-performance mode to a power-saving mode, which intentionally slows down your processor to conserve energy. This impacts everything, but Chrome feels it especially hard because web browsers are resource-intensive applications.

Second, Windows updates can interfere with graphics drivers. Chrome uses your graphics card to render pages quickly, and when drivers get updated or reset, you might experience lag, choppy scrolling, or pages taking forever to load.

Third, background services that start with Windows can compete for the limited resources on slower computers. After an update, Windows might enable new services or restart old ones, leaving even less memory available for your browser.
...
Click on the battery icon in the bottom right corner of your screen, or open Settings and search for Power. Look for Power Mode or Power Plan. You want this set to Best Performance or High Performance. If it is set to Power Saver, change it immediately.
...
Right-click on the Start button and select Device Manager. Expand the Display adapters section to see your graphics card. It will likely be NVIDIA, AMD, or Intel. Visit the manufacturer's website to download the latest driver for your specific card.
...
Open Chrome, click on the three dots in the top right corner, and go to Settings. Scroll down and click on Privacy and security, then Clear browsing data. Select All time as the time range, make sure Cached images and files is checked, and click Clear data.
...
Go to Chrome settings and search for Hardware. Turn off Use hardware acceleration when available, then restart Chrome. This forces Chrome to use your processor instead of your graphics card, which can actually be faster in some situations, especially if your graphics drivers are having issues.
...
Open Chrome in incognito mode by clicking on the three dots and selecting New incognito window. Incognito mode disables all extensions by default. If Chrome runs smoothly in incognito mode, one of your extensions is causing the problem.
...
First, reduce the number of startup programs. Open Task Manager by pressing Ctrl+Shift+Esc, click on the Startup tab, and disable programs you do not need to run at startup.
...
This will reset everything, including your extensions, themes, and settings. Make sure to back up any important data first.

Alternatively, you can uninstall and reinstall Chrome completely. Go to Settings, find Apps, locate Chrome, and uninstall it. Then download the latest version from the official Chrome website and install it fresh.
