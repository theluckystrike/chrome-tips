...
description: "Is Chrome running slow during Zoom screen sharing? Discover step-by-step solutions to free up RAM and restore performance on older computers. Read more to optim"
date: 2026-01-15
...
This double workload quickly uses up your available RAM. Chrome is already one of the most memory-hungry browsers, and having many tabs open makes it worse. When your computer runs out of RAM, it starts using the hard drive as temporary memory, which is much slower. The result is choppy screen sharing and a laggy experience.
...
To enable Memory Saver, open Chrome and click the three dots in the top right corner. Go to Settings, then click Performance, and toggle Memory Saver on. You can also choose which sites you want to keep active always, in case you need certain tabs to stay fully loaded.
...
When a tab is suspended, it stops using CPU and RAM until you click on it again. The tab shows a placeholder instead of the full webpage, which reminds you it is suspended. This is a great way to keep your research tabs available without them slowing down your computer during important meetings.
...
Turn off any extensions that are not essential for your current work. This includes ad blockers, weather apps, shopping finders, and any other extensions you rarely use. Each extension runs in the background and consumes RAM, so disabling them can give you a noticeable performance boost.
...
On Windows, you can see what is running by pressing Ctrl+Shift+Esc to open Task Manager. On Mac, use the Activity Monitor to check which applications are using resources. Close anything that is not essential for your meeting.
...
- Make sure Memory Saver is enabled
- Consider using Tab Suspender Pro for extra memory savings
- Disable unused Chrome extensions
- Close other applications running in the background
- Restart Chrome for a fresh session

Following these steps should significantly reduce Chrome slow when screen sharing Zoom, even on older computers with limited RAM. You do not need expensive hardware to have smooth video calls. You just need to manage your resources wisely.

Remember, the key is to reduce the memory burden on your computer before the meeting starts. By keeping Chrome lightweight and closing other applications, you give your computer the breathing room it needs to handle screen sharing without lag.
