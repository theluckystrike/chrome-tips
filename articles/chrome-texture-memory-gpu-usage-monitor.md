...
On macOS, the Activity Monitor application offers GPU monitoring capabilities. Access it through Applications > Utilities > Activity Monitor, then select the GPU tab to see usage and memory statistics. This is particularly useful for users with discrete graphics cards that Chrome may use for hardware acceleration.
