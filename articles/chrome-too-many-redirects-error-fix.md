...
description: "Learn how to fix the chrome too many redirects error with simple solutions. Clear cache, check extensions, and fix cookie issues. Read more to optimize your exp"
date: 2026-01-15
...
This can happen for several reasons. Sometimes the website itself has a configuration problem. The server might be stuck in a loop between two or more pages. This is not something you can fix on your end, but there are still things you can try.
...
To clear your cache and cookies in Chrome, click on the three dots in the upper right corner of the browser window. This opens the Chrome menu. From there, select "Settings" and then look for "Privacy and security" on the left side of the screen. Click on "Clear browsing data." Make sure both "Cookies and other site data" and "Cached images and files" are selected. You can choose a time range, but selecting "All time" is often the safest bet when dealing with redirect issues. Click "Clear data" and wait for the process to finish.
...
To check if an extension is causing the problem, try opening Chrome in incognito mode. Incognito mode disables most extensions by default. If the website works properly in incognito mode, one of your extensions is likely the culprit.

To identify which extension is causing trouble, go back to your regular Chrome window and click on the three dots, then select "Extensions" and "Manage Extensions." Turn off each extension one by one, testing the website after disabling each one. When the website starts working, you have found the problematic extension. You can then remove it or keep it disabled for that particular site.
...
You can try changing your DNS settings to use a public DNS service like Google DNS or Cloudflare DNS. On Windows, you can do this through the Network and Internet settings. On Mac, you would go to System Preferences, then Network, select your active connection, and click "Advanced." Go to the DNS tab and add new DNS server addresses.
...
On Windows, open the Command Prompt as an administrator and type "ipconfig /flushdns" and press Enter. You should see a message confirming that the DNS resolver cache was flushed successfully.

On Mac, open the Terminal app and type "sudo dscacheutil -flushcache" and press Enter. You may need to enter your password. Alternatively, you can try "sudo killall -HUP mDNSResponder" which also works on newer versions of macOS.
...
You can check if a website is having issues by visiting a status page like DownDetector or by searching on social media to see if others are reporting the same problem. If the website is indeed the problem, there is not much you can do except wait for the site administrators to fix it.
...
To reset Chrome, go to Settings and click on "Reset settings" on the left side. Click on "Restore settings to their original defaults" and then "Reset settings" to confirm. After the reset, try visiting the website again.
