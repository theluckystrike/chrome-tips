...
Modern websites are heavier than ever before. A single page might contain dozens of images, videos, embedded social media posts, advertisements, and tracking scripts. All of these elements need memory to load and stay ready for interaction. When you have multiple tabs open, Chrome has to keep all of this data in RAM, and 4GB fills up fast.
...
To enable Memory Saver, open Chrome and click the three dots in the top right corner. Go to Settings, then find the Performance section. Toggle Memory Saver on. You can also choose which sites should never be paused, such as music players or video streaming services that need to keep running in the background.
...
Tab Suspender Pro is especially helpful for 4GB laptop users because it allows you to create custom rules based on how you browse. You can set tabs to suspend after one minute of inactivity, or you can configure it to always keep certain sites active. The extension also shows you exactly how much memory each tab is using, helping you identify the biggest memory hogs in your browsing session.
...
Try to develop a habit of closing tabs you no longer need. If you find yourself keeping tabs open for later, consider using bookmarks instead. You can organize bookmarks into folders and retrieve them whenever you need them, without keeping them resident in memory. Chrome also has a feature that shows you your recently closed tabs, so you can always find your way back to pages you accidentally closed.
...
Take some time to review your installed extensions and remove any that you do not use regularly. To do this, type chrome://extensions in your address bar and look through the list. Ask yourself when you last used each extension and whether it provides enough value to justify its memory usage. If you have not used an extension in months, it is probably safe to remove it.
...
Go to Chrome Settings and find the On Startup section. Choose "Open the New Tab page" instead of "Continue where you left off." This gives you control over which tabs to open fresh each time you start Chrome, rather than automatically loading everything from your previous session. You can still manually restore previous tabs if needed through the history menu, but you will not be forced to load them all at startup.
