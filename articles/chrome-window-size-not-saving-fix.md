...
description: "Is Chrome forgetting your window size every time you reopen it? Learn why this happens and how to fix it with simple solutions. Read more to optimize your exper"
date: 2026-01-15
...
One of the most frequent reasons is that Chrome is not closing properly. If Chrome crashes or is force-closed while open, any window size changes you made might not get saved to the browser's preferences. The next time you open Chrome, it loads the last saved state, which might be from a previous session.

Another common cause is having multiple Chrome windows open when you close the browser. Chrome saves the state of all open windows, but if there are inconsistencies or conflicts between different window states, the browser might not remember your preferred size correctly. Sometimes an update to Chrome can also reset or corrupt these settings, causing the browser to revert to default window sizes.

Extensions can also interfere with window size saving. Some extensions, particularly those that manage tabs or windows, can conflict with Chrome's built-in window state saving mechanism. If you recently installed a new extension, that might be why your window size is not sticking.
...
First, make sure you are closing Chrome properly. Click the X button in the top right corner and wait for Chrome to close completely before opening it again. Do not just minimize the browser or let it run in the background. If you use the taskbar on Windows or the dock on Mac, make sure Chrome is fully quit and not just hidden.

Try closing Chrome and then opening it again. Sometimes the browser gets into a state where it is not saving changes properly, and a fresh start can reset this behavior. Make sure all Chrome windows are closed, including any that might be running in the background.
...
On Windows, make sure Chrome is not running as an administrator. Running as administrator can sometimes prevent the browser from saving settings properly. Right-click your Chrome shortcut and check the properties. If it says "Run as administrator" in the compatibility tab, uncheck that option.

On Mac, make sure Chrome has permission to access your files and folders. Go to System Settings, then Privacy and Security, and check that Chrome has the necessary permissions. Sometimes resetting these permissions can fix window-related issues.
...
Click the three dots in the top right corner of Chrome and go to Settings. Scroll down and click on Privacy and security, then click Clear browsing data. In the window that appears, select the options for Cached images and files and Cookies and other site data. You can choose to clear data for all time or just the past hour to see if that helps.

After clearing the data, close Chrome completely and reopen it. Set your window to the size you prefer, then close Chrome properly. Reopen it and see if the size has been saved.
...
To reset Chrome, click the three dots in the top right corner and go to Settings. On the left side of the page, look for the option that says Reset settings. Click on it, then click the button that says Restore settings to their original defaults. Confirm by clicking Reset settings again.
...
To test this, open Chrome and go to Settings. Click Extensions on the left side. Try removing extensions one by one, testing the window size after each removal. Start with any extensions you added recently, as those are most likely to be causing the problem.
...
Check if there is a newer version of Chrome available. Click the three dots, go to Help, and click About Chrome. Chrome will check for updates and install them if available. After updating, close and reopen Chrome, then test if the window size saves correctly.
